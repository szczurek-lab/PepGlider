...
class DecoderRNN(nn.Module):
    def __init__(self, latent_size, hidden_size, output_size, num_layers=1):
        ...
        self.lstm = nn.LSTM(hidden_size, hidden_size, num_layers, batch_first=False, bidirectional=True, device=DEVICE)
    ...
    def forward(self, z, hidden):
        ...
        output = self.fc(z)#.unsqueeze(1)
        batch_size = z.size(1)
        num_directions = 2  # For bidirectional LSTM
        hidden_size = self.lstm.hidden_size
        hx = torch.zeros(self.lstm.num_layers * num_directions, batch_size, hidden_size).to(z.device)
        cx = torch.zeros(self.lstm.num_layers * num_directions, batch_size, hidden_size).to(z.device)
        
        output, _ = self.lstm(output, (hx, cx))
    ...
    # def __init__(self, z_size, # n_conditions, condition_size, 
    #              hidden_size, output_size, n_layers=1, word_dropout=1.):
    #     super(DecoderRNN, self).__init__()
    #     self.output_size = output_size
    #     self.n_layers = n_layers
    ...
    #     if max_sample:
    #         # Sample top value only - it's enough, but
    ...
    #     outputs = Variable(torch.zeros(n_steps, 1, self.output_size)).to(device)
    #     input = Variable(torch.LongTensor([64])).to(device)
...
class VAE(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        output_sequence = []
        for i in range(25):
            # Assuming `mu` is the mean vector output from your encoder
            mu = torch.randn(1, 100).to(DEVICE)
            hidden = self.decoder.init_hidden(1, DEVICE)
            # Move the decoder call outside the loop
            with torch.no_grad():
                output, hidden = self.decoder(mu, hidden)
            # Assuming you want to select the most probable character at each step
            _, max_indices = torch.max(output, dim=1)
            output_sequence.append(max_indices.tolist())
            if len(output_sequence) == 25:
                break

        # Convert the output_sequence to letter sequence using your mapping
        # output_letter_sequence = [index_to_char[i] for i in output_sequence]
        # print(output_letter_sequence)

        # x_hat = self.decoder(z, None, y, temperature, DEVICE)
        output_tensor = torch.tensor(output_sequence)
        x = x.view(-1)#.float()
        loss_function = nn.CrossEntropyLoss()
        loss = loss_function(output_tensor[0][:25], x)
