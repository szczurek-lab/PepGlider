...
def save_model(model: nn.Module, name: str, with_hash: bool = True) -> None:
    if with_hash:
        ...
        model_name = name
    save(
        model.module.state_dict(), (MODELS_DIR / model_name).with_suffix(".pt")
    )
...
def run_epoch_iwae(
    ...
    for batch, labels in dataloader:       
        ...
        for dim in reg_dim:
            ...
            z.reshape(-1,z.shape[2])[indexes,:], physchem_decoded[dim], dim, gamma=gamma, factor=1.0 #gamma i delta z papera
        )
